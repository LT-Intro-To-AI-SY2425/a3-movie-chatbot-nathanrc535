...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    result = []
    for movie in movie_db:
        if get_year(movie) >= year1 and get_year(movie) <= year2:
            result.append(get_title(movie))
    return result
...
def year_by_title(matches: List[str]) -> List[int]:
    ...
    for movie in movie_db:
        if get_title(movie) == movie1:
            result.append(int(get_year(movie)))
    return result
...
def doesListContainCoolWords(curr: List[str]) -> List[str]:
    ### list checks if list contains ALL words: screen, ring, pen, and king
    ### If yes, return "This is such a cool list of words!"
    ### If no, return "This list contains gross words."
    word1 = "screen"
    word2 = "ring"
    word3 = "pen"
    word4 = "king"
    ...
    if pass1:
        if pass2:
            if pass3:
                if pass4:
                    return ["This is such a cool list of words!"]
    return ["This list contains gross words."]
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("are there cool words in %"), doesListContainCoolWords),
    (["bye"], bye_action),
]
...
def search_pa_list(src: List[str]) -> List[str]:
    ...
    """
    if src==["bye"]:
        return bye_action("bugerking")
    for i in pa_list:
        if match(i[0],src):
            answer=i[1](match(i[0],src))
            return ["No answers"] if answer==[] else answer
